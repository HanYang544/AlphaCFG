...
from tree import TokenMap, compute_expression_similarity, expression_to_prefix_str

from .alpha_pool import AlphaPoolBase
from data.calculator__ import AlphaCalculator, TensorAlphaCalculator
from data.expression import Expression, OutOfDataRangeError
from data.pool_update import PoolUpdate, AddRemoveAlphas
from data.correlation import batch_pearsonr
...
class LinearAlphaPool(AlphaPoolBase, metaclass=ABCMeta):
    ...
    def force_load_expr(self, expr: Expression, weights: Optional[List[float]] = None) -> None:
        """
        Add a single expression to the pool. If the pool exceeds capacity, 
        remove the expression with the smallest weight.
        
        Args:
            expr: The expression to be added to the pool.
            weights: Optional weights for the expressions in the pool.
        """
        self._failure_cache = set()
        ...
        # If the pool is not full, simply add the new expression.

        if self.size < self.capacity:
            try:
                ic_ret, ic_mut = self._calc_ics(expr, ic_mut_threshold=None)
            except (OutOfDataRangeError, TypeError):
                return  # If an error occurs, skip adding this expr.
            
            assert ic_ret is not None and ic_mut is not None
            self._add_factor(expr, ic_ret, ic_mut)
            added.append(expr)
        else:
            # If the pool is full, remove the expression with the smallest weight.
            ...
            # Find the expression with the smallest weight and remove it.
            min_weight_idx = np.argmin(self.weights)
            remaining_indices = [i for i in range(self.size) if i != min_weight_idx]
            self.leave_only(remaining_indices)
    ...
    def initialize(self, expression_list: List[Expression]) -> None:
        for expression in expression_list :
            self.try_new_expr((expression))
...
class MseAlphaPool(LinearAlphaPool):
    ...
    def compute_avg_similarity_with_pool(self, expr_str: str) -> float:
        ...
        for expr in self.exprs[:self.size]:
            if expr is None:
                continue

            expr_in_pool_str = expression_to_prefix_str(expr)

            if compute_expression_similarity(expr_str, expr_in_pool_str, token_map)>=sim:
                sim = compute_expression_similarity(expr_str, expr_in_pool_str, token_map)
