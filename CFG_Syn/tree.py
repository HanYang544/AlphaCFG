def compute_expression_similarity(expr_str1, expr_str2, token_map=None):
    ...
    Parameters:
        ...
        token_map: Optional parameter, if not provided, a new TokenMap instance will be created internally
...
# The following is internal code used by the function
...
class TokenMap:
    ...
    def get_arity(self, operator):
        """
        Return the number of arguments for the operator, defaulting to 0 (undefined arity).
        """
...
def common_subtree_size(t1, t2):
    """
    If t1 and t2 are isomorphic, return the size of their common subtree (number of nodes), otherwise return 0.
    Here, isomorphism requires that node values are the same, the number of children is the same, and the order is consistent.
    """
...
def normalize_similarity(tree1, tree2):
    """
    Normalize the maximum common subtree size with the normalization formula:
...
def expression_to_prefix_str(expr: Expression) -> str:
    """
    Convert an Expression class instance to a prefix expression string.
    ...
    # If it's a feature (Feature), directly call its __str__ method (usually returns a string like "$xxx")
    ...
    # If it's a paired rolling operator (PairRollingOperator)
        ...
        # For other cases, directly use str(expr)
        return str(expr)
