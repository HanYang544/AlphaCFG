...
class Std(RollingOperator):
    def _apply(self, operand: torch.Tensor) -> torch.Tensor:
        dim = -1
        dim_size = operand.size(dim)
        
        if dim_size > 1:
            # 正常计算标准差，使用无偏估计
            return operand.std(dim=dim, unbiased=True)
        elif dim_size == 1:
            # 只有一个元素，标准差为零
            zero_std = torch.zeros(operand.size()[:-1], dtype=operand.dtype, device=operand.device)
            logging.warning(f"operand.size({dim}) = {dim_size}，标准差设为零。")
            return zero_std
        else:
            # 没有元素，返回零张量或其他默认值
            zero_std = torch.zeros(operand.size()[:-1], dtype=operand.dtype, device=operand.device)
            logging.warning(f"operand.size({dim}) = {dim_size}，无元素，标准差设为零。")
            return zero_std
...
class Var(RollingOperator):
    def _apply(self, operand: torch.Tensor) -> torch.Tensor:
        ...
        if dim_size > 1:
            # 正常计算方差，使用无偏估计
            return operand.var(dim=dim, unbiased=True)
        elif dim_size == 1:
            # 只有一个元素，方差为零
            zero_var = torch.zeros(operand.size()[:-1], dtype=operand.dtype, device=operand.device)
            logging.warning(f"operand.size({dim}) = {dim_size}，方差设为零。")
            return zero_var
        else:
            # 没有元素，返回零张量或其他默认值
            zero_var = torch.zeros(operand.size()[:-1], dtype=operand.dtype, device=operand.device)
            logging.warning(f"operand.size({dim}) = {dim_size}，无元素，方差设为零。")
            return zero_var
