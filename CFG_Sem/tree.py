def compute_expression_similarity(expr_str1, expr_str2, token_map=None):
    ...
    Returns:
        A float value representing the normalized similarity score, ranging from 0 to 1.
    """
...
def common_subtree_size(t1, t2):
    """
    If t1 and t2 are isomorphic, return their common subtree size (number of nodes), otherwise return 0.
    Isomorphism here assumes that node values are the same, number of children is the same, and order is consistent.
    """
...
def normalize_similarity(tree1, tree2):
    """
    Normalize the maximum common subtree size with the formula:
...
def expression_to_prefix_str(expr: Expression) -> str:
    ...
    # If it's a rolling operator (RollingOperator), assuming the second parameter is window length
