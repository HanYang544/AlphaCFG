...
def set_seed(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
    torch.backends.cudnn.deterministic = True  # Ensure convolution algorithm is deterministic
    torch.backends.cudnn.benchmark = False     # If the input dimensions or types vary little, set True might be better

def setup_logging():
    # Remove existing handlers to prevent duplication or overwriting
...
def main():
    ...
    try:
        # Device configuration, modify here to ensure correct GPU usage
        ...
        vocab_size = len(ENV_TOKEN_MAP) + 1
        ...
        # Combine all parameters and create optimizer
        ...
        logger.debug("Replay buffer initialization completed.")
        ...
        # =============== New: Save pool_dict as a JSON file ===============
        ...
        if not os.path.exists(pool_file):
            with open(pool_file, 'w', encoding='utf-8') as f:
                pass  # Write an empty file to avoid errors
        # ============================================================
        ...
        for iteration in range(1, NUM_ITERATIONS + 1):
            ...
            for num in range(1, NUM + 1):
                ...
                all_game_data = parallel_self_play(
                    ...
                    device=device,
                    num_workers=mp.cpu_count()//2  # Use half of the CPU cores 
                    )
            # ============================================================
            ...
            logger.info(f"Average policy loss for iteration {iteration}: {avg_policy_loss}")

            # Train value network
            avg_value_loss = train_value_net(value_net, optimizer, replay_buffer, NUM_BATCHES, BATCH_SIZE, device)
            logger.info(f"Average value loss for iteration {iteration}: {avg_value_loss}")
            ...
            if pool.size > 0:
                    # =============== Get pool dict and save to file ===============
                    ...
                    # Save results along with iteration to logs/pool_dicts.json
            ...
            # Clear replay buffer for next self-play data
            if no_improve_counter >= 0.2 * NUM_ITERATIONS:
                logger.info(f"ic_value_valid has not improved for {0.2 * NUM_ITERATIONS} iterations, early stopping training at iteration {iteration}.")
                break
        ...
        logger.exception("Unhandled exception occurred during training")
