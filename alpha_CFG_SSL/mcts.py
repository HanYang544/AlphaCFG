...
def apply_dirichlet_noise(root, epsilon=0.25, alpha=0.03):
    ...
    epsilon: Noise ratio, controlling the combination ratio of original priors and noise
    alpha: Parameter of the Dirichlet distribution, controlling the noise distribution
    """
    ...
    # Iterate through each child node and modify its prior probability
...
class Expression:
    def __init__(self, state, continue_out_game):
        self.state = state
        self.continue_out_game = continue_out_game

    def __str__(self):
        return f"{self.state_description()} | continue_out_game: {self.continue_out_game}"


class Node:
    def __init__(self, state=AlphaEnv(), parent=None, proba=1, action=None, continue_out_game=3):
        """
        ...
        self.childrens = {}  # Child nodes, with action tuples as keys and nodes as values
        self.parent = parent
        self.continue_out_game = continue_out_game
    ...
    def expand(self, probas):
        ...
        for action_output in probas:
            ...
            if prior_prob > 0:
                ...
                if new_state.expansion_depth<=LEN-3:
                    continue_out_game = 3
                else:
                    continue_out_game = LEN-new_state.expansion_depth
    ...
    def run(self):
        for sim in range(self.mcts.MCTS_SIM // self.mcts.MCTS_PARALLEL):
            ...
            while len(self.result_queue) < self.mcts.MCTS_PARALLEL:
                ...
                expressions = list(self.eval_queue.values())[0:max_len]  # Get expressions to evaluate

                # Get the value (v) and policy probabilities (probas) for each expression
                v = [evaluate_expression_value(expression.state, value_net=self.value_net) for expression in expressions]
                probas = [evaluate_policy_network(expression.state, policy_net=self.policy_net, continue_out_game=expression.continue_out_game)
...
class SearchThread(threading.Thread):
    ...
    def run(self):
        ...
        # Continue searching while the current node is expanded and not a terminal state
        while current_node.childrens and not current_node.state.is_terminal_state():
            # If the current node has no child nodes, treat it as a leaf node and stop searching
        ...
        # Originally constructed the expression like this:
        # expression = current_node.state, current_node.state.continue_out_game
        expression = Expression(current_node.state.state_description(), current_node.continue_out_game)
...
class MCTS:
    ...
    def calculate_action_probs_from_node2(self):
        ...
        """
        # Get the visit counts of the child nodes of the current node
    ...
    def advance(self, move):
        ...
        # Use move as the key to access child nodes, rather than using index idx
    ...
    def search(self, competitive, c_puct, temperature):
        """
        Search for the best action through game tree search, updating node statistics with the policy network and value network
        """
        ...
        ## Decide the action selection method based on whether it's competitive
        if competitive:
            ...
            # If final_move is still not assigned after the loop, choose the last action as a fallback
...
if __name__ == "__main__":
    ...
    #seed = 3  # You can choose any number you like
    #set_seed(seed)
