...
def parse_expression_from_string(expression_str: str) -> Expression:
    ...
    Parameters:
    - expression_str: str, the prefix notation expression, e.g., "Add $high $low"

    Returns:
    - Expression object
    """
    tokens = expression_str.split()
    tokens = tokens[::-1]  # Reverse the list for easier pop() usage from the end
    ...
    def parse() -> Expression:
        ...
        # Feature variables
            ...
            feature_name = token[1:].upper()  # Assume the FeatureType names are uppercase
            try:
    ...
    expression = parse()
    if tokens:
        raise ValueError("Invalid expression: there are extra tokens.")
    return expression
